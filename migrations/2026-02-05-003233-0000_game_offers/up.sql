...
CREATE TABLE offers(
    ...
    FOREIGN KEY (made_to) REFERENCES users(id)
);
...
CREATE POLICY "Users cannot offer games to themselves"
ON offers
AS RESTRICTIVE
FOR INSERT
WITH CHECK ( (SELECT current_setting('app.current_user_id', true)::integer) != made_to);
...
CREATE POLICY "User being offered to must own their game"
ON offers
AS RESTRICTIVE
FOR INSERT
WITH CHECK ( EXISTS (SELECT 1 FROM games WHERE owned_by = made_to AND id = for_game) );
...
CREATE POLICY "Offerees can update their own offers."
ON offers
AS PERMISSIVE
FOR UPDATE
USING ( (SELECT current_setting('app.current_user_id', true)::integer) = made_to)
WITH CHECK ( (SELECT current_setting('app.current_user_id', true)::integer) = made_to);
...
CREATE POLICY "Offeree offers cannot be updated to be somebody else's game."
ON offers
AS RESTRICTIVE
FOR UPDATE
WITH CHECK ( EXISTS (SELECT 1 FROM games WHERE owned_by = made_to AND id = for_game) );
...
CREATE POLICY "Offerees can delete their offers."
ON offers FOR DELETE
USING ( (SELECT current_setting('app.current_user_id', true)::integer) = made_to);
